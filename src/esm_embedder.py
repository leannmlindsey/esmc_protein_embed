...
class ESMCEmbedder(BaseEmbedder):
    ...
    def load_model(self):
        print(f"Loading ESM-C model '{self.model_name}' on {self.device}...")
        ...
        try:
            ...
            try:
                self.model = ESMC.from_pretrained(self.model_name, trust_remote_code=True).to(self.device)
                self.model.eval()
                print(f"Model '{self.model_name}' loaded successfully!")
            except Exception as e2:
                raise ValueError(f"Failed to load model '{self.model_name}'. Error: {str(e2)}")
